...
{	
	...
	public static function defaultConstants()
	{
		...
		{
			$uniqueKey = mb_substr( md5( 'weadown' . '$Rev: 3023$'), 10, 10 );
		}
		...
		return array(
			...
			// to change them.
				...
				'CACHEBUST_KEY'		=> mb_substr( md5( '23f57f32' . '$Rev: 3023$'), 10, 10 ),
	...
	{
		$ourUrl = str_replace(['http://', 'https://', 'www.'], '', Settings::i()->base_url);
		$fakeData = [
			'fetched' => time(),
			'data' => [
				'expires' => date('Y-m-d', strtotime('+360 days')),  // License expires in 30 days
				'active' => true,
				'url' => $ourUrl,
				'legacy' => false,
				'products' => [
		...
		$cached = Store::i()->license_data ?? null;
		if ( isset( $cached ) && !$forceRefresh && $cached['fetched'] > ( time() - 1814400 ) )
		{
			return $cached['data'];
		}
		$response = $fakeData['data'];
		Store::i()->license_data = [
			'fetched' => $fakeData['fetched'],
			'data' => $response
		];
