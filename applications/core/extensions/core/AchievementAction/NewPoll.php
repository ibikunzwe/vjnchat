 ...
 */
class NewPoll extends AchievementActionAbstract // NOTE: Other classes exist to provided bases for common situations, like where node-based filters will be required
{	
	/**
	...
	{
		$nthFilter = new Number( 'achievement_filter_poll_nth', ( $filters and isset( $filters['milestone'] ) and $filters['milestone'] ) ? $filters['milestone'] : 0, FALSE, [], NULL, Member::loggedIn()->language()->addToStack('achievement_filter_nth_their'), Member::loggedIn()->language()->addToStack('achievement_filter_NewPoll_nth_suffix') );
	...
	public function formatFilterValues( array $values ): array
	{
		...
		if ( isset( $values['achievement_filter_poll_nth'] ) )
		{
			$return['milestone'] = $values['achievement_filter_poll_nth'];
		}
		return $return;
	}
	...
	public function filtersMatch( Member $subject, array $filters, mixed $extra = NULL ): bool
	{
		...
		{
			$where = [];
			$where[] = [ 'starter_id=?', $subject->member_id ];
					
			$count = Db::i()->select( 'COUNT(*)', 'core_polls', $where )->first();
	...
	{
		return Member::loggedIn()->language()->addToStack( 'AchievementAction__NewPoll_log', FALSE );
	}
	...
	public function ruleDescription( Rule $rule ): ?string
	{
		...
		if ( isset( $rule->filters['milestone'] ) )
		{
			$conditions[] = Member::loggedIn()->language()->addToStack( 'achievements_title_filter_milestone', FALSE, [
				...
				'sprintf'		=> [ Member::loggedIn()->language()->addToStack('AchievementAction__NewPoll_title_generic') ]
			] );
		}

		return Theme::i()->getTemplate( 'achievements' )->ruleDescription(
			Member::loggedIn()->language()->addToStack( 'AchievementAction__NewPoll_title' ),
			$conditions
		);
	}
	...
	{
		Member::load( $row['starter_id'] )->achievementAction( 'core', 'NewPoll', Poll::load( $row['pid'] ) );
	}
}