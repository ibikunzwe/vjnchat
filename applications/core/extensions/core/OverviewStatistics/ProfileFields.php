...
class ProfileFields extends OverviewStatisticsAbstract
{
	 ...
	 * @return	string
	 */
	protected function _getChart( array $field ) : string
	{
		/* Init Chart */
		$pieBarData = array();
		$results	= array();
		...
		{
			$pieBarData[] = array(
				'name' =>  Member::loggedIn()->language()->addToStack('stats_pfields__unchecked'),
			...
			$pieBarData[] = array(
				'name' =>  Member::loggedIn()->language()->addToStack('stats_pfields__checked'),
		...
		{
			$pieBarData[] = array(
				'name' =>  Member::loggedIn()->language()->addToStack('stats_pfields__no'),
				'value' => isset( $results[0] ) ? $results[0]['total'] : 0,
				'percentage' => ( isset( $results[0] ) AND $results[0]['total'] > 0 ) ? round( ( $results[0]['total'] / $total ) * 100, 2 ) : 0
			);

			$pieBarData[] = array(
				'name' =>  Member::loggedIn()->language()->addToStack('stats_pfields__yes'),
				'value' => isset( $results[1] ) ? $results[1]['total'] : 0,
				'percentage' => ( isset( $results[1] ) AND $results[1]['total'] > 0 ) ? round( ( $results[1]['total'] / $total ) * 100, 2 ) : 0
			);
		}
