 ...
 * @brief	Overview statistics extension: RepReceived
 */
class RepReceived extends OverviewStatisticsAbstract
{
	/**
	...
	public function getBlock( array|string $dateRange = NULL, string $subBlock = NULL ): string
	{
		...
		if( $dateRange !== NULL )
		{
			...
			else
			{
				...
				$previousCount = Db::i()->select( 'COUNT(DISTINCT(member_received))', 'core_reputation_index', array( array( 'rep_date BETWEEN ? AND ?', $currentDate->sub( $interval )->getTimestamp(), $initialTimestamp ) ) )->first();
			}
		}

		$count = Db::i()->select( 'COUNT(DISTINCT(member_received))', 'core_reputation_index', $where )->first();
