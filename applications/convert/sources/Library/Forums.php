...
class Forums extends Core
{
			...
			switch( $k )
			{
				...
				case 'convertForumsQuestionRatings':
					$return[ $k ] = array(
						...
						'ips_rows'		=> Db::i()->select( 'COUNT(*)', 'forums_question_ratings' ),
						'source_rows'	=> array( 'table' => $v['table'], 'where' => $v['where'] ),
						'per_cycle'		=> 200,
						'dependencies'	=> array( 'convertForumsTopics' ),
				...
				case 'convertForumsAnswerRatings':
					$return[ $k ] = array(
						...
						'ips_rows'		=> Db::i()->select( 'COUNT(*)', 'forums_answer_ratings' ),
						'source_rows'	=> array( 'table' => $v['table'], 'where' => $v['where'] ),
						'per_cycle'		=> 200,
	...
	protected function truncate( string $method ) : array
	{
		...
		foreach( $classname::canConvert() as $k => $v )
		{
			switch( $k )
			{
				...
				case 'convertForumsQuestionRatings':
					$return['convertForumsQuestionRatings'] = array( 'forums_question_ratings' => NULL );
					break;
	...
	public function convertForumsForum( array $info=array(), ?array $perms=NULL, ?string $iconpath=NULL, ?string $icondata=NULL ) : bool|int
	{
		...
		if ( !isset( $info['seo_last_title'] ) )
		{
			...
			{
				$info['seo_last_title'] = Url::seoTitle( $info['last_title'] );
			}
			else
			{
				$info['seo_last_title'] = '';
		...
		if ( !isset( $info['seo_last_name'] ) )
		{
			...
			{
				$info['seo_last_name'] = Url::seoTitle( $info['last_poster_name'] );
			}
			else
			{
				$info['seo_last_name'] = '';
	 ...
	 *
	 * @param	array			$info	Data to insert
	 * @return	int|bool	The ID of the newly inserted rating, or FALSE on failure.
	 */
	public function convertForumsQuestionRating( array $info=array() ) : bool|int
	{
		...
		if ( isset( $info['topic'] ) )
		{
			...
			{
				$this->software->app->log( 'question_rating_missing_question', __METHOD__, App::LOG_WARNING, ( $hasId ) ? $info['id'] : NULL );
				return FALSE;
			}
		}
		else
		...
		if ( isset( $info['forum'] ) )
		{
			...
			{
				$this->software->app->log( 'question_rating_missing_forum', __METHOD__, App::LOG_WARNING, ( $hasId ) ? $info['id'] : NULL );
				return FALSE;
			}
		}
		else
		...
		if ( isset( $info['member'] ) )
		{
			...
			{
				$this->software->app->log( 'question_rating_missing_member', __METHOD__, App::LOG_WARNING, ( $hasId ) ? $info['id'] : NULL );
				return FALSE;
			}
		}
		else
		...
		{
			$this->software->app->log( 'question_rating_missing_rating', __METHOD__, App::LOG_WARNING, ( $hasId ) ? $info['id'] : NULL );
			return FALSE;
		}
		else
		{
			...
			{
				$this->software->app->log( 'question_rating_invalid_rating', __METHOD__, App::LOG_WARNING, ( $hasId ) ? $info['id'] : NULL );
				return FALSE;
			}
		}
		...
		$inserted_id = Db::i()->insert( 'forums_question_ratings', $info, TRUE );
	 ...
	 * @return	int|bool	The ID of the newly inserted rating, or FALSE on failure.
	 */
	public function convertForumsAnswerRating( array $info=array() ) : bool|int
	{
		...
		if ( isset( $info['post'] ) )
		{
			...
			{
				$this->software->app->log( 'answer_rating_missing_answer', __METHOD__, App::LOG_WARNING, ( $hasId ) ? $info['id'] : NULL );
				return FALSE;
			}
		}
		else
		...
		if ( isset( $info['member'] ) )
		{
			...
			{
				$this->software->app->log( 'answer_rating_missing_member', __METHOD__, App::LOG_WARNING, ( $hasId ) ? $info['id'] : NULL );
				return FALSE;
			}
		}
		else
		...
		if ( !isset( $info['rating'] ) )
		{
			$this->software->app->log( 'answer_rating_missing_rating', __METHOD__, App::LOG_WARNING, ( $hasId ) ? $info['id'] : NULL );
			return FALSE;
		}
		else
		{
			...
			{
				$this->software->app->log( 'answer_rating_invalid_rating', __METHOD__, App::LOG_WARNING, ( $hasId ) ? $info['id'] : NULL );
				return FALSE;
			}
		}
		...
		if ( isset( $info['topic'] ) )
		{
			...
			{
				$this->software->app->log( 'answer_rating_missing_topic', __METHOD__, App::LOG_WARNING, ( $hasId ) ? $info['id'] : NULL );
				return FALSE;
			}
		}
		else
		...
		$inserted_id = Db::i()->insert( 'forums_answer_ratings', $info, TRUE );
